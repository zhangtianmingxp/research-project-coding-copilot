...
### 5.4 大规模执行前必须 Pilot-first

任何大规模数据处理、模型推理、外部 API 调用或正式 benchmark 前，必须先做小规模 pilot / dry-run。
...
- 支持断点恢复，避免失败后从头执行；
- 密钥、token 和 credential 不得写入代码、日志、Markdown 或 Git；
- 正式运行前先确认缓存复用和失败重试策略。
...
### 9.5 长期项目必须维护主线摘要和证据索引

当项目持续多个阶段或超过约 10-20 个重要 prompt/result 轮次时，应维护：

- 当前研究主线摘要；
- 推荐阅读顺序；
- 分阶段 workflow checkpoint；
- 关键决策和负结果记录；
...
后续 AI 应优先读取这些压缩文档和最近 1-3 个相关 result，避免重复加载全部历史。
