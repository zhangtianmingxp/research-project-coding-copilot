...
- 连续两轮主要是工程补强或 smoke 时，下一轮原则上必须产生科研决策级证据，除非存在已写明的具体 blocker。
...
工程要求应与科学风险和复用需求相称。探索性分析可以保持简单，只要输入、方法、输出和解释边界可追踪；稳定且支撑核心结论的逻辑再沉淀为正式模块。
...
2. 只解除完成这些比较所必需的数据或 pipeline blocker；
3. 尽快在具有科研辨别力的规模上运行统计基线、强基线和核心分析；
4. 对有希望的 claim 补充严格 split、不确定性、消融、对照、重复和外部验证；
5. 只有在能够检验科学假设或改善论文关键结果时才引入高级模型；
6. 完成机制分析、领域案例和 claim-to-figure 故事；
7. 对最终证据所依赖的流程做复现加固、文档化和交付整理。
